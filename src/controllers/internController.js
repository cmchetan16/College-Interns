...
const createIntern = async (req, res) => {
    try {
        ...
        if (!validFullName(name))
            ...
            const dublicateEmail = await internModel.findOne({email:email})
            if(dublicateEmail) return res.status(400).send({ status: false, msg: "email already registered" })
        ...
        const dublicateMobile = await internModel.findOne({mobile:mobile})
        if(dublicateMobile) return res.status(400).send({ status: false, msg: "mobile already registered" })
            

        const collegeNames = await collegeModel.findOne({ $or: [{ name: collegeName }, { fullName: collegeName }] })
        if (!collegeNames) { return res.status(404).send({ status: false, msg: "College not found" }) }
